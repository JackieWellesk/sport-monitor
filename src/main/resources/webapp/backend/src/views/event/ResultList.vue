              ...
              <el-form-item label-width="">
                <el-row :justify="isMobile ? 'end' : 'start'" style="width: 100%;">
                  <el-space>
                    ...
                    <el-button type="success" :loading="loading" @click="goCreate">
                      登记成绩
                    </el-button>
                  </el-space>
                </el-row>
...
function goCreate() {
  // 新增登记：不带 id
  router.push({ path: '/events/results/edit' })
}

function goEdit(row) {
  // 修改：必须带 id
  router.push({ path: '/events/results/edit', query: { id: row.id } })
}
...
async function deleteRow(row) {
  ...
  try {
    // 你按自己后端实际接口改这里
    await request.delete(`/api/results/${row.id}`)
